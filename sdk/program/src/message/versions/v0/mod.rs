...
fn get_keys(instructions: &[Instruction], payer: Option<&Pubkey>, address_lookup_tables: Option<&[AddressLookupTable]>) -> InstructionKeys {
    ...
    let mut address_table_lookups: Vec<MessageAddressTableLookup> = address_lookup_tables.unwrap_or(&vec![]).iter().map(|table| MessageAddressTableLookup {
    ...
    for account_meta in &unique_metas {
        if account_meta.is_signer {
            ...
            if let Some((table_index, key_index)) = find_first_address_table_pubkey(&account_meta.pubkey, address_lookup_tables) {
                if account_meta.is_writable {
                    address_table_lookups[table_index].writable_indexes.push(key_index);
                } else {
                    address_table_lookups[table_index].readonly_indexes.push(key_index);
                }
...
fn find_first_address_table_pubkey(pubkey: &Pubkey, address_lookup_tables: Option<&[AddressLookupTable]>) -> Option<(usize, u8)> {
    if address_lookup_tables.is_none() {
        return None;
    }
    address_lookup_tables.unwrap().iter().enumerate().find_map(|(table_index, table)| table.addresses.iter().enumerate().find_map(|(key_index, key)| if key == pubkey { Some((table_index, key_index as u8)) } else { None }))
}
...
impl Message {
    ...
    pub fn new_with_payer(instructions: &[Instruction], payer: Option<&Pubkey>, address_lookup_tables: Option<&[AddressLookupTable]>) -> Self {
    ...
    pub fn new_with_blockhash(
        ...
        println!("keys: {:?} {:?} {:?}", &signed_keys, &unsigned_keys, &all_keys);
        println!("instr: {:?}", &instructions[0]);
